...
class Parameter {
      ...
      if (this.spec.schema.hasOwnProperty("type")) {
        switch(this.spec.schema.type) {
          case "string":
            ...
            break;
          case "number":
            value = this.coerceNumber(this.spec.schema);
            break;
          case "integer":
            value = this.coerceNumber(this.spec.schema, true);
            break;
          case "object":
  ...
  coerceString(spec) {
    ...
    if (spec.hasOwnProperty("pattern")) {
      const reg = new RegEx(spec.pattern);
      // TODO consider best way to handle this.
      // Since here we are coercing, and this may work better for validation
    }
  ...
  coerceNumber(spec, isInteger = false) {
    let value = null;
    ...
    if (spec.hasOwnProperty("multipleOf")) {

    }

    if (spec.hasOwnProperty("minimum")) {
      if ((value >= spec.minimum) == false) {
        return null;
      }
    }

    if (spec.hasOwnProperty("exclusiveMinimum")) {
      if ((value > spec.exclusiveMaximum) == false) {
        return null;
      }
    }

    if (spec.hasOwnProperty("maximum")) {
      if ((value <= spec.maximum) == false) {
        return null;
      }
    }

    if (spec.hasOwnProperty("exclusiveMaximum")) {
      if ((value < spec.exclusiveMaximum) == false) {
        return null;
      }
    }
